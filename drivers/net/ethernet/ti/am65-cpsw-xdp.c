...
{
/* As RX/TX queues (flows) are shared between ports we can no longer
 * support the case where Zero copy is enabled for the queue id for
 * one port but not for others.
 * Current solution will be to just drop the packet if Zero copy
 * is not enabled for that port/qid but enabled for qid on another port.
 */
	...
	/* As channels/flows are shared between ports, we can only allow
	 * one port to use any queue as XSK at a time.
	 */
